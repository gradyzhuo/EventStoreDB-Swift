...
public struct ClientSettings {
    ...
    public func makeCallOptions() throws -> CallOptions {
        var options = CallOptions.init()
        if let user = self.defaultUserCredentials {
            options.customMetadata.replaceOrAdd(name: "Authorization", value: try user.basicAuthHeader)
        }
        return options
...
extension ClientSettings {
    ...
    public enum TopologyClusterMode{
        ...
        static func gossipCluster(endpoints: [Endpoint], nodePreference: NodePreference)->Self{
            return .gossipCluster(endpoints: endpoints, nodePreference: nodePreference, timeout: DEFAULT_GOSSIP_TIMEOUT)
        }
    }
...
extension ClientSettings : ExpressibleByStringLiteral {
    ...
    public init(stringLiteral value: String) {
        ...
        }catch ClientSettingsError.parseError(let message){
