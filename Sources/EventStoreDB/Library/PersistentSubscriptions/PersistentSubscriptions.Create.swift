...
extension PersistentSubscriptions.Create {
    public struct FilterOption{
        let filter: Stream.Read.SubscriptionFilter
        let checkpointIntervalMultiplier: UInt32
        
        
        public init(filter: Stream.Read.SubscriptionFilter, checkpointIntervalMultiplier: UInt32) {
            self.filter = filter
    ...
    public enum StreamSelection {
        ...
        func build(options: inout PersistentSubscriptions.Create.Request.UnderlyingMessage.Options) throws{
