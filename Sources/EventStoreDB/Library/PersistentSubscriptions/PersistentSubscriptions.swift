...
public struct PersistentSubscriptions: _GRPCClient {
    ...
    public init(selection: StreamSelection, groupName: String, settings: ClientSettings = EventStore.shared.settings) throws{
...
extension PersistentSubscriptions{
    ...
    //MARK: - Create Action
    public static func createOn(streamSelection selection: Create.StreamSelection, groupName: String, options: Create.Options, settings: ClientSettings = EventStore.shared.settings) async throws -> Self{
    ...
    public static func createOn(streamSelection selection: Create.StreamSelection, groupName: String, settings: ClientSettings = EventStore.shared.settings, configure: (_ options: Create.Options)->Create.Options) async throws -> Self{
    ...
    public func read(bufferSize: Int32 = 500, uuidOption: UUID.Option = .string, options: Read.Options) async throws -> AsyncStream<Read.Result> {
        ...
        let responses =  underlyingClient.read(requests)
...
//        }
    ...
    public func ack(eventIds: [UUID], subscriptionId: String? = nil) async throws {
        ...
        let rs = self.underlyingClient.read(requests)
    ...
    public static func list(options: List.Options, settings: ClientSettings = EventStore.shared.settings) async throws -> [SubscriptionInfo]{
        let channel = try GRPCChannelPool.with(settings: settings)
    ...
    public static func list(settings: ClientSettings = EventStore.shared.settings, configure: (_ options: List.Options) -> List.Options) async throws -> [SubscriptionInfo]{
...
extension PersistentSubscriptions{
    ...
    public struct SubscriptionInfo: GRPCBridge {
        ...
        init(from message: UnderlyingMessage){
            ...
            let connections: [PersistentSubscriptions.ConnectionInfo] = message.connections.map{
                .init(
                    from: $0.from,
                    username: $0.username,
